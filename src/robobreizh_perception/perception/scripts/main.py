...
class Perception:
	...
	def continuous_detection(self)
		...
		while(1):
			...
			img = rospy.wait_for_message("/pepper/camera/front/image_raw", Image) 
			imageF = self.bridge.imgmsg_to_cv2(img, "bgr8")
    		obj = ["all"]
    		result = self.image_request(sock, imageF, obj, True)
    		image_res = self.base64_to_cv2(result["image_object"])
    		cv2.imwrite("../images/detection.png",image_res)
    		cv2.imshow('image_result',image_res)
